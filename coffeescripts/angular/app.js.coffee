...
myApp.controller 'MainCtrl', ['$scope','$timeout','$sce', ($scope, $timeout, $sce) ->
...
20130122, R+')
  ...
  editor_1_html.getSession().on('change', (e)->
    $scope.bindHtml1 = $sce.trustAsHtml(editor_1_html.getValue())
    $scope.$apply()
  )
...
#wrap>span.id{\n
  ...
  editor_1_css.getSession().on('change', (e)->
    $scope.bindCss1 = $sce.trustAsHtml(editor_1_css.getValue())
    $scope.$apply()
  )
  # Slide_1 End
...
<section>\n
  ...
  editor_2_html.getSession().on('change', (e)->
    $scope.bindHtml2 = $sce.trustAsHtml(editor_2_html.getValue())
    $scope.$apply()
  )
...
.tip span:before, .tip span:after {\n
  ...
  editor_2_css.getSession().on('change', (e)->
    $scope.bindCss2 = $sce.trustAsHtml(editor_2_css.getValue())
    $scope.$apply()
  )
  # Slide_2 End
  ...
  editor_3_html.getSession().on('change', (e)->
    $scope.bindHtml3 = $sce.trustAsHtml(editor_3_html.getValue())
    $scope.$apply()
  )
...
.check-circle.black {\n
  ...
  editor_3_css.getSession().on('change', (e)->
    $scope.bindCss3 = $sce.trustAsHtml(editor_3_css.getValue())
    $scope.$apply()
  )
  # Slide_3 End
